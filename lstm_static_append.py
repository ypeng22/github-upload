...
#traindata1 = np.zeros((len(traindata), 539, n_features + n_static))
#testdata1 = np.zeros((len(testdata), 539, n_features + n_static))
#append static to time series
#for i in range(len(traindata)):
#    for j in range(539):
#        traindata1[i][j] = np.concatenate([traindata[i][j], trainSD[i]])
        
#for i in range(len(testdata)):
#    for j in range(539):
#        testdata1[i][j] = np.concatenate([testdata[i][j], testSD[i]])        
##np.save('traindata1', traindata1)
#np.save('testdata1', testdata1)

dev = torch.device("cuda:0")
mv_net = lstm(n_features + n_static,n_timesteps)
mv_net.to(dev)
...
for t in range(epoch):
    for b in range(0,len(traindata),batch_size):
        inpt = traindata1[b:b+batch_size,:,:]
